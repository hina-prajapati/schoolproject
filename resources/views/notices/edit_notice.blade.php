...
<!--         
    ...
    @foreach ($existingFiles as $noticeId => $files)
    <p>Existing Files for Notice ID: {{ $noticeId }}</p>
    <ul>
        <!-- @foreach ($files as $file)
            <li>
                @if (in_array(strtolower(pathinfo($file->filename, PATHINFO_EXTENSION)), ['pdf', 'jpg', 'jpeg', 'png', 'doc', 'docx', 'zip']))
...
@endforeach
                ...
                <div class="form-group">
                  ...
                  <input type="text" name="title" value="<?=$notice['title']?>" class="form-control" aria-describedby="emailHelp" placeholder="Enter title">
                  
                </div>

                <div class="form-group">
                  <label for="exampleInputPassword1">Short Description</label>
                  <input type="text" name="short_description" value="<?=$notice['short_description']?>" class="form-control"  placeholder="Password">
...
<textarea rows="3" class="mb-3 d-none" name="job_description" id="quill-editor-area">{!! $notice->job_description !!}</textarea>
