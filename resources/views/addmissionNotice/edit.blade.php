  ...
  <nav>
    ...
    <form action="" method="POST" enctype="multipart/form-data" style="overflow:scroll ">
        ...
        <div class="form-group mt-2">
            ...
            @if(isset($admission['thumbnail']))
            <p>Current Image:</p>
            <img src="{{ asset('/singleImageAdmission/' . $admission['thumbnail']) }}" alt="Current Thumbnail" class="w-25">
        @endif
            ...
            <!-- @foreach ($existingFiles as $admissionId => $files)
        <p>Existing Files for Admission ID: {{ $admissionId }}</p>
        <ul>
            @foreach ($files as $file)
                <li>
                    @if (in_array(strtolower(pathinfo($file->filename, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png']))
                        <img src="{{ asset('/admission/' . $file->filename) }}" width="50" alt="Image">
                    @else
                        <a href="{{ asset('/admission/' . $file->filename) }}" target="_blank">{{ $file->filename }}</a>
                    @endif
...
@endforeach
                ...
                <div class="form-group">
                  ...
                  <input type="text" name="title" value="<?=$admission['title']?>" class="form-control" aria-describedby="emailHelp" placeholder="Enter title">
                  
                </div>

                <div class="form-group">
                  <label for="exampleInputPassword1">Short Description</label>
                  <input type="text" name="short_description" value="<?=$admission['short_description']?>" class="form-control"  placeholder="Password">
                 
                </div>

                <div class="form-group">
                    <label for="exampleInputPassword1">Long Description</label>
                    <input type="text" name="long_description" value="<?=$admission['long_description']?>" class="form-control"  placeholder="Password">
                  
                  </div>

                  <div id="quill-editor" class="mb-3" style="height: 300px;">
    {!! $admission->job_description !!}
</div>
<textarea rows="3" class="mb-3 d-none" name="job_description" id="quill-editor-area">{!! $admission->job_description !!}</textarea>
